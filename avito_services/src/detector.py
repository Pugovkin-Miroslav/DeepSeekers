...
def _heuristic_detect(description: str) -> Set[int]:
    """
    Эвристическое определение микрокатегорий с учетом контекста.
    Исключает ложные срабатывания типа "штукатурка под обои" -> обои.
    """
    normalized = normalize_text(description)
    detected_ids = set()
    
    # Приоритетные фразы с высоким доверием
    priority_phrases = {
        101: ["ремонт под ключ", "комплексный ремонт", "полный ремонт", "комплекс работ", "генподряд"],
        102: ["сантехника", "сантехнические работы", "монтаж сантехники", "разводка труб", "опрессовка труб", "демонтаж сантехники", "монтаж бойлера"],
        103: ["электрика", "электромонтажные работы", "замена проводки", "электрощит", "электромонтаж", "штробление стен"],
        104: ["натяжные потолки", "натяжной потолок", "тканевые потолки", "пвх потолок", "потолок в коридоре"],
        ...
        110: ["гипсокартон", "гкл", "монтаж гипсокартона", "перегородки из гипсокартона", "декоративные конструкции из гкл", "монтаж перегородок"],
    ...
    # Дополнительная проверка по ключевым фразам из словаря с фильтрацией ложных срабатываний
    for mc in MICROCATALOG:
        if mc.mcId in detected_ids:
            continue
        
        # Фразы, которые часто дают ложные срабатывания в пассивном контексте
        passive_triggers = {
            106: ["обои", "обоями", "обоев"],  # "штукатурка под обои" != поклейка обоев
            104: ["потолок", "потолки"],  # "потолок в коридоре" может быть частью ремонта, но не обязательно натяжной
            109: ["пол", "пола"],  # "напольные покрытия" в общем контексте
            103: ["электрика", "электрику"],  # "электрика" как часть комплекса
            102: ["сантехника", "сантехнику"],  # "сантехника" как часть комплекса
        }
        
        found_valid_phrase = False
        for phrase in mc.keyPhrases:
            norm_phrase = normalize_text(phrase)
            if len(norm_phrase) < 4:
                continue
            
            # Проверка на пассивный контекст
            is_passive = False
            if mc.mcId in passive_triggers and norm_phrase in passive_triggers[mc.mcId]:
                # Проверяем, не стоит ли фраза в пассивном контексте
                patterns = [
                    rf"под\s+{re.escape(norm_phrase)}",
                    rf"для\s+{re.escape(norm_phrase)}",
                    rf"после\s+{re.escape(norm_phrase)}",
                    rf"перед\s+{re.escape(norm_phrase)}",
                ]
            ...
            # Для коротких общих фраз требуем дополнительного подтверждения
            if len(norm_phrase.split()) < 2:
                specific_terms = {
                    102: ["бойлер", "водонагреватель", "полотенцесушитель", "канализация", "водоснабжение", "опрессовка"],
                    103: ["штробление", "электромонтаж", "проводка", "электрощит"],
                    104: ["натяжной", "натяжных", "пвх", "тканевый"],
                    105: ["керамогранит", "мозаика", "кафель", "плиточный"],
                    106: ["поклейка", "оклейка", "клеим", "флизелиновые", "виниловые", "стеклообои"],
                    107: ["малярка", "короед", "венецианская", "шпаклевка"],
                    108: ["штукатурка", "маяки", "механизированная"],
                    109: ["ковролин", "линолеум", "пробка", "кварцвинил", "наливной", "паркет", "ламинат"],
                    110: ["гкл", "гвл", "гипрок", "перегородки"],
                    111: ["демонтаж", "снос", "перфоратор"],
                }
                if mc.mcId in specific_terms and norm_phrase not in specific_terms[mc.mcId]:
                    continue
            
            if norm_phrase in normalized:
                found_valid_phrase = True
                detected_ids.add(mc.mcId)
                break
    
    # Обработка разделенных частей через "/" - здесь более либерально, НО с фильтрацией
    if "/" in description:
        parts = description.split("/")
        for part in parts:
            part_normalized = normalize_text(part)
            # Пропускаем очень короткие части (менее 3 слов) - это часто просто слова-паразиты
            if len(part_normalized.split()) < 3:
                continue
            for mc in MICROCATALOG:
                if mc.mcId in detected_ids:
                    continue
...
def train_ml_model(data_path: str = None, save_model: bool = True) -> MLCategoryDetector:
    ...
    if data_path is None:
        possible_paths = [
            Path(__file__).parent.parent / "data" / "rnc_dataset_markup.csv",
            Path(__file__).parent / "data" / "rnc_dataset_markup.csv",
            Path("data") / "rnc_dataset_markup.csv",
        ]

...
def _heuristic_detect(description: str) -> Set[int]:
    ...
    # Расширенный словарь приоритетных фраз с более широким покрытием
    priority_phrases = {
        101: ["ремонт под ключ", "комплексный ремонт", "полный ремонт", "комплекс работ", "генподряд"],
        102: ["сантехника", "сантехнические работы", "монтаж сантехники", "разводка труб", "опрессовка труб", "демонтаж сантехники"],
        103: ["электрика", "электромонтажные работы", "замена проводки", "электрощит", "электромонтаж"],
        ...
        110: ["гипсокартон", "гкл", "монтаж гипсокартона", "перегородки из гипсокартона", "декоративные конструкции из гкл"],
    ...
    # Дополнительная проверка по ключевым фразам из словаря микрокатегорий
    for mc in MICROCATALOG:
        ...
        for phrase in mc.keyPhrases:
            ...
            if mc.mcId in skip_general and norm_phrase in skip_general[mc.mcId]:
                continue
            # Разрешаем короткие фразы для специфичных терминов
    ...
    # Обработка разделенных частей через "/"
    if "/" in description:
